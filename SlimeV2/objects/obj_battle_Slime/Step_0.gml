...
if(keyboard_check(vk_shift)){
	new_x = lerp(vpos_x, clamp(mouse_x - vpos_w - vpos_x, -vpos_w, vpos_w) + obj_battle_Slime.x - vpos_w + 32, rate);
	new_y = lerp(vpos_y, clamp(mouse_y - vpos_h - vpos_y, -vpos_h, vpos_h) + obj_battle_Slime.y - vpos_h + 32, rate);
} else {
//Interpolate the view position to the new, relative position.
	new_x = lerp(vpos_x, obj_battle_Slime.x - vpos_w + 32, rate);
	new_y = lerp(vpos_y, obj_battle_Slime.y - vpos_h + 32, rate);
}
